...
- compositing, transition, and mask workflow method names now exist. Swipe/copy-machine transitions respect extent, color, opacity, angle, width, and time; disintegrate transitions use mask-threshold timing and shadow parameters. Remaining transition filters still need deeper Core Image-equivalent semantics; tracked in [#9](https://github.com/eliheuer/drawbot-skia/issues/9).
