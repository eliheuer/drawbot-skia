...
- geometry and distortion method names now exist as Pillow-backed approximations. `displacementDistortion()` uses red and green channels for independent x/y displacement. `droste()` uses inset points, strands, periodicity, rotation, and zoom for recursive inset composition. `perspectiveCorrection()` respects crop=False by preserving the expanded transform canvas and offset. Nine-part stretch/tile filters now preserve breakpoint-defined edge and corner regions while expanding the center regions. Keystone and advanced warps still need deeper Core Image-equivalent behavior; tracked in [#10](https://github.com/eliheuer/drawbot-skia/issues/10).
