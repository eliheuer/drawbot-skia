...
- color/statistical filters now exist. CIE Lab conversion and Delta E use sRGB/D65 color math; KMeans returns a one-dimensional palette image with alpha weights; palettize and paletteCentroid use color-distance assignment against supplied palette images. Spot-color semantics still need deeper Core Image-equivalent behavior; tracked in [#7](https://github.com/eliheuer/drawbot-skia/issues/7).
