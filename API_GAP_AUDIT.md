...
```text
352 passed, 3 skipped, 3 warnings
```
...
- blur, stylization, saliency, segmentation, material, and lighting filters now exist as Pillow-backed approximations. `bokehBlur()` uses an aperture kernel with radius, ring amount, ring size, and softness controls. `depthOfField()` uses the point0/point1 focus line to preserve nearby detail and blur farther pixels. `guidedFilter()` uses radius-controlled smoothing and guide-edge preservation controlled by epsilon. `maskedVariableBlur()` uses mask luminance to select local blur radius. `shadedMaterial()` treats the shading image as a height field for relief lighting controlled by scale. `spotLight()` uses `lightPointsAt` as the beam target, applies directional falloff from `lightPosition`, and alpha-masks areas outside the highlight. The remaining filters still need deeper Core Image-equivalent behavior; tracked in [#8](https://github.com/eliheuer/drawbot-skia/issues/8).
