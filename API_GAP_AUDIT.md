...
- blur, stylization, saliency, segmentation, material, and lighting filters now exist as Pillow-backed approximations. `bokehBlur()` uses an aperture kernel with radius, ring amount, ring size, and softness controls. `guidedFilter()` uses radius-controlled smoothing and guide-edge preservation controlled by epsilon. `maskedVariableBlur()` uses mask luminance to select local blur radius. `spotLight()` uses `lightPointsAt` as the beam target, applies directional falloff from `lightPosition`, and alpha-masks areas outside the highlight. The remaining filters still need deeper Core Image-equivalent behavior; tracked in [#8](https://github.com/eliheuer/drawbot-skia/issues/8).
